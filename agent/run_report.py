...
- **No improvement is claimed across different test sets.** The baseline and
  the best iteration must share an evaluation window (`date_range` and
  `n_forecasts`). If they do not, `improvement_frac` is `None`,
  `windows_match` is False, both windows are printed, and the recommendation is
  "inconclusive" — a smaller WIS on a different set of forecasts is not a win.
- **Segment keys are strings, whatever the source.** `evaluate_by_horizon`
  returns int keys in-process and the tracker returns JSON strings; `_segment`
  normalizes both so the live report and `--rebuild` render the same tables.
...
class RunReport:
    ...
    date_range: dict[str, str]  # the best iteration's window (baseline's when best is 0)
    n_locations: int | None
    n_forecasts: int | None
    baseline_date_range: dict[str, str]
    baseline_n_forecasts: int | None
    windows_match: bool  # False => baseline and best were scored on different data
...
def _assemble_report(
    *,
    ...
    The headline `improvement_frac` is only meaningful when the baseline and the
    best iteration were scored on the same forecasts. If the evaluation window
    (date range or forecast count) differs between them, the improvement is
    withheld (`None`) and `windows_match` is False so the renderer can say so
    instead of printing a number that compares two different test sets.
    """
    ...
    baseline_range, baseline_n = _eval_window(baseline_metrics)
    best_range, best_n = _eval_window(best_metrics) if best_metrics else (baseline_range, baseline_n)
    windows_match = (baseline_range, baseline_n) == (best_range, best_n)

    improvement = None
    if windows_match and baseline_value is not None and best_value is not None:
    ...
    return RunReport(
        ...
        baseline_n_forecasts=baseline_n,
...
def _eval_window(metrics: dict[str, Any]) -> tuple[dict[str, str], int | None]:
    """(date_range, n_forecasts) of one metrics dict — the identity of its test set."""
    if not isinstance(metrics, dict):
        return {}, None
    date_range = metrics.get("date_range")
    overall = metrics.get("overall")
    n_forecasts = _as_int(overall.get("n_forecasts")) if isinstance(overall, dict) else None
    return (dict(date_range) if isinstance(date_range, dict) else {}), n_forecasts


def _window_span(date_range: dict[str, str], n_forecasts: int | None) -> str:
    n_fc = n_forecasts if n_forecasts is not None else "?"
    return f"{date_range.get('min', '?')} .. {date_range.get('max', '?')} ({n_fc} forecasts)"
...
def _segment(metrics: dict[str, Any], section: str, key: str) -> dict[str, Any]:
    """Look up one segment (a horizon or phase) by its *string* key.

    `PhaseEvaluator.evaluate_by_horizon` returns int keys in-process, while the
    tracker round-trips them through JSON as strings. `_segment_keys` always
    hands us strings, so normalize the block the same way before indexing —
    otherwise every live (non-rebuilt) report renders a blank horizon table.
    """
    block = metrics.get(section) if isinstance(metrics, dict) else None
    if not isinstance(block, dict):
        return {}
    normalized = {str(k): v for k, v in block.items()}
...
def _header_block(report: RunReport) -> list[str]:
    ...
    if not report.windows_match:
        out.append(
            f"- baseline window: {_window_span(report.baseline_date_range, report.baseline_n_forecasts)}"
        )
        out.append(
            f"- best-iteration window: {_window_span(report.date_range, report.n_forecasts)}"
        )
    if report.baseline_value is None or report.best_value is None:
        out.append("- baseline -> best: no scoreable iteration")
    elif not report.windows_match:
        out.append(
            f"- baseline -> best: {_metric_num(report.baseline_value, metric)} -> "
            f"{_metric_num(report.best_value, metric)} (not comparable: windows differ)"
        )
    else:
        out.append(
            f"- baseline -> best: {_metric_num(report.baseline_value, metric)} -> "
...
def _caveats(report: RunReport) -> list[str]:
    ...
    if not report.windows_match:
        out.append(
            "baseline and best iteration were scored on different evaluation windows "
            f"({_window_span(report.baseline_date_range, report.baseline_n_forecasts)} vs "
            f"{_window_span(report.date_range, report.n_forecasts)}); the headline "
            "improvement is withheld and the attribution tables compare different test sets."
        )
...
def _recommendation(report: RunReport) -> str:
    ...
    if not report.windows_match:
        return (
            f"inconclusive — baseline (iteration 0) and best (iteration {report.best_iteration}) "
            "were scored on different evaluation windows, so no improvement can be claimed"
        )
